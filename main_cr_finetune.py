...
from utils.for_3ds import MixUp3D, select_best_model
...
def get_args_parser():
    ...
    parser.add_argument('--eval_freq', default= 10, type= int, help = 'the evaluation frequence')
...
def main(args):
    ...
    y = [data[1] for data in whole_dataset]
    results = {}
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(whole_dataset, y)):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if args.output_dir and epoch == args.epochs - 1 and misc.is_main_process():
                misc.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                    loss_scaler=loss_scaler, epoch=epoch, post_name= f"_fold_{fold}_epoch_{epoch}")
            
            if args.use_3d:
                if epoch == args.epochs - 1 and misc.is_main_process():
