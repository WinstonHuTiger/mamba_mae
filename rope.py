...
class VisionRotaryEmbeddingFast(nn.Module):
    def __init__(
        ...
        if if_3d:
            freqs_ = repeat(freqs, '... n -> ... (n r)', r = 2)
            # print(freqs.shape)
            # freqs = broadcat((freqs[:, None,  :],), dim = -1
            freqs = broadcat((freqs_[:, None, :], freqs_[None, :, :]), dim = -1)
            freqs = broadcat((freqs[None,:, :, :, None], freqs[:, None, :, :, None]), dim = -1)
            freqs = freqs[:, :, :,:, 0]
        else:
